...
try {
    ...
    $employeeId = isset($parts[0]) ? (int)$parts[0] : 0;

    // Fallback to session if no valid userId from token
    if (!$employeeId && isset($_SESSION['user_id'])) {
    ...
    // 1) Fetch Employee User Info - IMPORTANT: Changed role check to 'employee'
    $userQuery = "
    ...
    if (!$userData) {
        ...
        echo json_encode(['error' => 'No employee found for this user']);
        exit;
    }
    ...
    $milestoneStmt->execute([$employeeId]);
    ...
    $overdueStmt->execute([$employeeId]);
    ...
    error_log("Employee Dashboard PDO Error: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Database error: ' . $e->getMessage()]);
    exit;
} catch (Exception $e) {
    error_log("Employee Dashboard General Error: " . $e->getMessage());
    http_response_code(500);
