...
$logFile = __DIR__ . '/trainee_practical_exams_log.txt';
...
// Allow trainee or administrator role
...
try {
  ...
  if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Query to get a specific practical exam for a trainee
    // Only show exams for programs the trainee is enrolled in
    $query = "
      SELECT 
        pe.id, 
        pe.program_id, 
      ...
      JOIN
        program_enrollments pen ON p.id = pen.program_id AND pen.user_id = :traineeId
      WHERE 
        pe.id = :examId
    ";
  ...
  // Query to get all practical exams for a trainee's enrolled programs
  $query = "
    SELECT 
      pe.id, 
      pe.program_id, 
  ...
  error_log("Database error in trainee/practical_exams.php: " . $e->getMessage());
  file_put_contents($logFile, "PDO Error: " . $e->getMessage() . "\n", FILE_APPEND);
  http_response_code(500);
  echo json_encode(['error' => 'Database error: ' . $e->getMessage()]);
} catch (Exception $e) {
  error_log("Error in trainee/practical_exams.php: " . $e->getMessage());
