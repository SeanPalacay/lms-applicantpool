    ...
    const ENDPOINTS = {
        DOCUMENTS: '/lms-forbes/backend/api/admin/documents.php',
...
// In your adminService.js, add better error handling
getJobPositions: async (department = '') => {
    try {
        const authToken = localStorage.getItem('authToken');
        if (!authToken) {
            console.error('No authToken found in localStorage');
        ...
        const url = `${API_BASE_URL}/lms-forbes/backend/api/admin/positions.php${department ? `?department=${encodeURIComponent(department)}` : ''}`;
        console.log('Fetching job positions from:', url);
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
        ...
        console.log('Raw positions response:', responseText);
        let data;
        try {
            data = JSON.parse(responseText);
        } catch (e) {
            console.error('Failed to parse positions response as JSON:', e);
        ...
        console.error('Error in getJobPositions:', error.message, error.stack);
        return [];
    }
},
    ...
    updateApplicationStatus: async (applicationId, status, reason = '') => {
        try {
            ...
            console.log('New status:', status, 'Reason:', reason);
            const response = await fetch(url, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
    ...
    getApplications: async (filters = {}) => {
        try {
            ...
            const data = await response.json();
            // Ensure applications are sorted by applied_at for FCFS
            return data.sort((a, b) => new Date(a.applied_at) - new Date(b.applied_at));
        } catch (error) {
    ...
    getWaitlistByPosition: async (positionId) => {
        try {
            const authToken = localStorage.getItem('authToken');
            if (!authToken) {
                console.error('No authToken found in localStorage');
            ...
            const url = `${API_BASE_URL}/lms-forbes/backend/api/admin/applications.php?position_id=${positionId}&status=waitlisted`;
            console.log('Fetching waitlist for position from:', url);
            const response = await fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Response error details:', errorText);
                throw new Error(`Failed to fetch waitlist: ${response.status} ${response.statusText} - ${errorText}`);
            }
            const data = await response.json();
            // Sort by applied_at for FCFS
            return data.sort((a, b) => new Date(a.applied_at) - new Date(b.applied_at));
        } catch (error) {
            console.error(`Error in getWaitlistByPosition(${positionId}):`, error.message, error.stack);
            return [];
        }
    },

    moveNextWaitlistedToApplied: async (positionId) => {
        try {
            const authToken = localStorage.getItem('authToken');
            if (!authToken) {
                console.error('No authToken found in localStorage');
            ...
            const url = `${API_BASE_URL}/lms-forbes/backend/api/admin/applications.php?action=move_waitlisted&position_id=${positionId}`;
            console.log('Moving next waitlisted applicant to Applied at:', url);
            const response = await fetch(url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Response error details:', errorText);
                throw new Error(`Failed to move waitlisted applicant: ${errorText}`);
            }
            return await response.json();
        } catch (error) {
            console.error(`Error in moveNextWaitlistedToApplied(${positionId}):`, error.message, error.stack);
            throw error;
        }
    },

    getLeaderboardData: async (poolId) => {
        try {
            const authToken = localStorage.getItem('authToken');
            if (!authToken) {
                console.error('No authToken found in localStorage');
            ...
            const url = `${API_BASE_URL}/lms-forbes/backend/api/admin/leaderboard.php?pool_id=${poolId}`;
            ...
            console.error('Error in getLeaderboardData:', error.message, error.stack);
            throw error;
        }
    },
    ...
    getTraineesByPool: async (poolId) => {
        try {
            ...
            if (!poolId || isNaN(poolId)) {
                console.error('Invalid poolId provided:', poolId);
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('API error response for pool trainees:', errorText);
                if (response.status === 401) {
                    localStorage.removeItem('authToken');
                    throw new Error('Authentication failed. Please login again.');
                }
            ...
            // Ensure data is an array and contains expected fields
            if (!Array.isArray(data)) {
                console.warn('Expected array of trainees, received:', data);
                return [];
            }
            // Validate that each trainee object has required fields
            const validatedData = data.map(trainee => ({
                user_id: trainee.user_id || null,
                full_name: trainee.full_name || 'Unknown',
                program_id: trainee.program_id || null,
                program_name: trainee.program_name || 'N/A',
                batch_id: trainee.batch_id || null,
                role: trainee.role || 'trainee',
                status: trainee.status || 'active',
                added_at: trainee.added_at || new Date().toISOString()
            }));
    ...
    removeTraineeFromPool: async (traineeId, poolId) => {
        try {
            const authToken = localStorage.getItem('authToken');
            if (!authToken) {
                console.error('No authToken found in localStorage');
            ...
            const url = `${API_BASE_URL}/lms-forbes/backend/api/admin/trainee-pools-trainees.php?trainee_id=${traineeId}&pool_id=${poolId}`;
    ...
    getTraineesByProgram: async (programId) => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/lms-forbes/backend/api/admin/trainee-pool-assignments.php?program_id=${programId}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${authToken}`,
                    'Content-Type': 'application/json'
            ...
            console.error(`Error in getTraineesByProgram(programId: ${programId}):`, error.message);
            throw error;
        }
    },

    // Add a trainee to a pool
    addTraineeToPool: async (poolId, userId) => {
        try {
            const authToken = localStorage.getItem('authToken');
            ...
            console.log('Request data:', { pool_id: poolId, user_id: userId });
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify({ pool_id: poolId, user_id: userId })
            });
            ...
            console.error(`Error in addTraineeToPool(poolId: ${poolId}, userId: ${userId}):`, error.message, error.stack);
            throw error;
        }
    },
    // Fetch all programs
    ...
    getUserDocuments: async (applicantId) => {
        try {
            ...
            const url = `${API_BASE_URL}${ENDPOINTS.DOCUMENTS}?applicant_id=${applicantId}`;
    ...
    downloadUserDocument: async (documentId) => {
        try {
            ...
            const url = `${API_BASE_URL}${ENDPOINTS.DOCUMENTS}?id=${documentId}&action=download`;
            ...
            if (!response.ok) {
                ...
                const errorMessage = errorData.error || 'Failed to download document';
                if (response.status === 401) {
                    localStorage.removeItem('authToken');
                    throw new Error('Authentication failed. Please log in again.');
                }
    ...
    viewUserDocument: async (documentId) => {
        try {
            ...
            const url = `${API_BASE_URL}${ENDPOINTS.DOCUMENTS}?id=${documentId}&action=view`;
            console.log('Viewing document at:', url);

            // Fetch the document as a blob with the Authorization header
            const response = await fetch(url, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${authToken}`,
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                const errorMessage = errorData.error || 'Failed to view document';
                if (response.status === 401) {
                    localStorage.removeItem('authToken');
                    throw new Error('Authentication failed. Please log in again.');
                }
            ...
            if (!newTab) {
                window.URL.revokeObjectURL(blobUrl); // Clean up if opening fails
                throw new Error('Failed to open document. Please allow pop-ups for this site.');
            }
