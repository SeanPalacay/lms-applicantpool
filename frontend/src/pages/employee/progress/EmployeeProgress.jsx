...
const ProgressTracking = () => {
  ...
  const fetchProgressData = async () => {
    try {
      ...
      const enrollmentsData = await employeeService.getPrograms();
      const quizAttemptsData = await employeeService.getQuizAttempts();
      const milestonesData = await employeeService.getMilestones();

      // Validate data
      setEnrollments(Array.isArray(enrollmentsData) ? enrollmentsData : []);
      setQuizAttempts(Array.isArray(quizAttemptsData) ? quizAttemptsData : []);
  ...
  const exportProgressPDF = async () => {
    try {
      const blob = await employeeService.exportProgressPDF();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'Employee_Progress_Report.pdf');
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      console.error('Error exporting progress:', err);
      setError('Failed to export progress report. Please try again.');
    }
  };
  ...
  const getFilteredEnrollments = () => {
    ...
    if (filterActive === 'completed') {
      filtered = filtered.filter(e => e.completion_status === 'completed');
    } else if (filterActive === 'in_progress') {
      filtered = filtered.filter(e => e.completion_status === 'in_progress');
    } else if (filterActive === 'not_started') {
      filtered = filtered.filter(e => e.completion_status === 'not_started');
    }

    if (sortBy === 'recent') {
      filtered.sort((a, b) => new Date(b.enrollment_date || 0) - new Date(a.enrollment_date || 0));
          ...
          <div style={{ display: 'grid', gap: 'var(--spacing-md)' }}>
            {filteredEnrollments.map(enrollment => (
              <div key={enrollment.id} style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-md)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)' }}>
                    ...
                    <span style={{ 
                      ...
                      {enrollment.completion_status?.replace('_', ' ') || 'Unknown'}
                    </span>
                  </div>
                </div>
                ...
                <Link 
                  to={`/employee/programs/${enrollment.program_id || enrollment.id}`} 
                  style={{ 
                    display: 'flex', 
                    alignItems: 'center', 
                    justifyContent: 'center', 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 'var(--spacing-md)', fontSize: '14px', color: 'var(--text-secondary)' }}>
                      ...
                      <span><Clock size={14} /> Due: {milestone.due_date ? new Date(milestone.due_date).toLocaleDateString() : 'Not set'}</span>
                    </div>
                  </div>
            ...
            <div style={{ display: 'grid', gap: 'var(--spacing-md)' }}>
              {quizAttempts.map(attempt => (
                <div key={attempt.id} style={{ 
                  ...
                  <div>
                    <Link 
                      to={`/employee/assessments/quiz/${attempt.quiz_id}/feedback?attempt=${attempt.id}`} 
                      style={{ 
                        display: 'flex', 
                        alignItems: 'center', 
                        justifyContent: 'center', 
