...
import employeeService from '../../../services/employeeService'; // Import employee service instead
import LoadingSpinner from '../../../components/shared/LoadingSpinner';
import AlertBanner from '../../../components/shared/AlertBanner';

const EmployeePracticalExams = () => {  // Renamed component
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (programId) {
          try {
            const programData = await employeeService.getProgramDetails(programId);
        ...
        const examsData = await employeeService.getPracticalExams(programId);
        setExams(examsData);
        
        // Fetch all employee's attempts
        const attemptsData = await employeeService.getPracticalExamAttempts();
    ...
    <div style={{ padding: '20px', backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          {programId && (
            <Link 
              to={`/employee/programs/${programId}`} // Updated route path to employee
              style={{ 
                display: 'inline-flex', 
                alignItems: 'center', 
                gap: '5px', 
              ...
              {exams.map(exam => {
                ...
                return (
                  <div key={exam.id} style={{ 
                    ...
                    <Link 
                      to={isGraded 
                        ? `/employee/practical-exams/${exam.id}/results/${attempt.id}` // Updated routes to employee
                        : isPending 
                          ? `/employee/practical-exams/${exam.id}/pending` 
                          : `/employee/practical-exams/${exam.id}`
                      } 
                      style={{ 
                        display: 'block', 
                        textAlign: 'center', 
...
export default EmployeePracticalExams;