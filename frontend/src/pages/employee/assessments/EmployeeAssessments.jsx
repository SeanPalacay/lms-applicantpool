...
import employeeService from '../../../services/employeeService'; // Changed to employeeService

const EmployeeAssessments = () => { // Changed component name
  ...
  useEffect(() => {
    const fetchAssessments = async () => {
      ...
      try {
        const data = await employeeService.getAssessments(); // Changed to employeeService
            ...
            return (
              ...
              >
                ...
                <div style={{ marginTop: '15px', marginLeft: '34px' }}>
                  {status === 'pending' ? (
                    <Link 
                      to={`/employee/assessments/quiz/${assessment.id}`} // Changed route URL
                      style={{ 
                        padding: '8px 15px', 
                        background: '#007bff', 
                        color: 'white', 
                    ...
                    <Link 
                    to={`/employee/assessments/quiz/${assessment.id}/feedback${latestAttempt ? `?attempt=${assessment.attempts[0].id}` : ''}`} // Changed route URL
                    style={{ 
                      padding: '8px 15px', 
                      background: '#6c757d', 
                      color: 'white', 
