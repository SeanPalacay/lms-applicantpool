...
const EditMilestone = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await trainerService.updateMilestone(milestoneId, { ...formData, trainees: selectedTrainees });
      setSuccessMessage('Milestone updated successfully!');
      setTimeout(() => navigate(`/trainer/milestones/${milestoneId}`), 2000);
    } catch (err) {
  ...
  const handleCancel = () => navigate(`/trainer/milestones/${milestoneId}`);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ color: '#ffffff', fontFamily: 'Inter, sans-serif' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 600, margin: 0 }}>Edit Milestone</h3>
          </div>
        </div>
