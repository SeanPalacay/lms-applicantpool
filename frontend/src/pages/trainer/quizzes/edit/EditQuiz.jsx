...
const EditQuiz = () => {
    ...
    const fetchQuizData = async () => {
      ...
      try {
        ...
        if (!quizResponse.questions || quizResponse.questions.length === 0) {
          ...
          // Transform questions to match our format if needed
          quizResponse.questions = quizResponse.questions.map(q => {
            const question = { ...q };
            
            // Handle specific question types
            if (q.question_type === 'true_false') {
              question.is_true = q.correct_answer === 'a';
            }
  ...
  const handleQuestionChange = (index, field, value) => {
    ...
    if (field === 'question_type') {
      ...
      if (newType === 'true_false') {
        ...
        // Reset to multiple choice or essay
        updatedQuestions[index] = {
          ...updatedQuestions[index],
          question_type: newType
        };
      ...
      const formData = {
        ...
        questions: quizData.questions.map(q => {
          ...
          if (q.question_type === 'multiple_answer') {
            question.correct_answer = q.correct_answers ? q.correct_answers.join(',') : '';
          } else if (q.question_type === 'identification') {
            question.correct_answer = q.answer_text;
          } else if (q.question_type === 'matching') {
            // We'll handle matching pairs separately in the backend
          }
  ...
  // Multiple answer question renderer
// Multiple answer question renderer
...
// Identification question renderer
